...
CPF's repository also tracks a generated single-header bundle at `include/cpflib`.

- add `includes/` to your compiler include path
- include `<cpflib>` for declarations and templates
...
`cpflib` is generated by recursively scanning the repository's `cpf/` source tree. It bundles both the public
