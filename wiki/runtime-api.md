...
## Single-header runtime distribution

CPF's repository also tracks a generated single-header bundle at `include/cpf.hpp`.

- add `includes/` to your compiler include path
- include `<cpf.hpp>` for declarations and templates
- define `CPF_IMPLEMENTATION` in exactly one translation unit to emit the runtime implementation bodies
...
`cpf.hpp` is generated by recursively scanning the repository's `cpf/` source tree. It bundles both the public
`cpflib` surface and the `runtime/runtime.h` detail declarations used by generated parser sources, so a single-header
consumer does not need the split `cpf/` include tree.
