...
namespace cpf {
   namespace {
      ...
      generated_code generate_code_impl(const grammar& grammar, const std::string& base_name,
         ...
         line(header, 0);
         line(header, 0,
               "inline void " + debug_block_helper_name + "(std::ostream& os, std::string block, std::size_t indent) {");
         line(header, 1, "if (block.empty()) {");
         line(header, 2, debug_indent_helper_name + "(os, indent);");
         line(header, 2, "return;");
         line(header, 1, "}");
         line(header, 1, "for (;;) {");
         line(header, 2, "const auto line_end = block.find('\\n');");
         line(header, 2, debug_indent_helper_name + "(os, indent);");
         line(header, 2, "os << block.substr(0, line_end);");
         line(header, 2, "if (line_end == std::string::npos) {");
         line(header, 3, "break;");
         line(header, 2, "}");
         line(header, 2, "os << '\\n';");
         line(header, 2, "block.erase(0, line_end + 1);");
         line(header, 2, "if (block.empty()) {");
         line(header, 3, debug_indent_helper_name + "(os, indent);");
         line(header, 3, "break;");
         line(header, 2, "}");
         line(header, 1, "}");
         line(header, 0, "}");
