...
namespace {
   ...
   [[nodiscard]] auto benchmark_size(const benchmark::BenchmarkReporter::Run& report) -> std::optional<benchmark::ComplexityN> {
	  ...
	  [[nodiscard]] auto milestone_key_for(const benchmark::BenchmarkReporter::Run& report) -> std::optional<std::string> {
		  ...
		  if (report.report_rms || report.run_type != benchmark::BenchmarkReporter::Run::RT_Aggregate || report.aggregate_name != "mean") {
			 return std::nullopt;
		  }
	  ...
	  bool ReportContext(const Context& context) override {
		 (void)context;
			 ...
			 if (!m_expected_milestones.empty()) {
				std::cerr << "[cpfbench] Progress 0/" << m_expected_milestones.size() << " (0.0%) elapsed 00:00" << std::endl;
			 }
		 return true;
	  }
			...
			if (report.aggregate_name == "mean") {
			   ...
			   if (auto items_per_second = find_items_per_second(report.counters); items_per_second.has_value()) {
				...
				if (milestone.has_value()) {
				   update_progress(*milestone, report.run_name.function_name, report.aggregate_name, benchmark_size(report));
				}
		 }
	  }
	  ...
	  void Finalize() override {
		 ...
		 for (auto family_name : family_order) {
			...
			output
			 ...
			 if (!m_expected_milestones.empty()) {
				std::cerr << "[cpfbench] Progress complete " << m_completed_milestones.size() << "/" << m_expected_milestones.size()
					     << " elapsed " << format_clock_duration(std::chrono::steady_clock::now() - m_started_at) << std::endl;
			 }
	  }

	private:
	  void update_progress(const std::string& milestone, std::string_view family_name, std::string_view aggregate_name, std::optional<benchmark::ComplexityN> size) {
		  ...
		  const auto percent = total == 0 ? 100.0 : (100.0 * static_cast<double>(completed) / static_cast<double>(total));
		  ...
		  std::cerr << "[cpfbench] Progress " << completed << "/" << total
			    << " (" << std::fixed << std::setprecision(1) << percent << "%)"
			    << " elapsed " << format_clock_duration(elapsed);

		  if (completed > 0 && total > completed) {
			 const auto average_seconds = std::chrono::duration<double>(elapsed).count() / static_cast<double>(completed);
			 const auto eta = std::chrono::duration_cast<std::chrono::steady_clock::duration>(std::chrono::duration<double>{average_seconds * static_cast<double>(total - completed)});
